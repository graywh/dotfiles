...
"      ^^^^^^^^^^^^^
"
syn match   xmlAttrib +[-'"<]\@<!\<[a-zA-Z:_][-.0-9a-zA-Z0-9:_]*\>\(['">]\@!\|$\)+ contained contains=xmlAttribPunct,@xmlAttribHook display
...
syn match   xmlNamespace +\(<\|</\)\@<=[^ /!?<>"':]\+[:]\@=+ contained contains=@xmlNamespaceHook transparent display
else
syn match   xmlNamespace +\(<\|</\)\@<=[^ /!?<>"':]\+[:]\@=+ contained contains=@xmlNamespaceHook display
endif
...
"  ^^^
"
syn match   xmlTagName +\(<\|</\)\@<=[^ /!?<>"']\++ contained contains=xmlNamespace,xmlAttribPunct,@xmlTagHook display
...
if exists('g:xml_syntax_folding')
    ...
    " s^^^^^^^^^^^^^^^e
    "
    syn region   xmlTag start=+<[^ /!?<>"']\@=+ end=+>+ contained contains=xmlTagError,xmlTagName,xmlAttrib,xmlEqual,xmlString,@xmlStartTagHook
    ...
    " ^^^^^^
    "
    syn region   xmlEndTag start=+</[^ /!?<>"']\@=+ end=+>+ contained contains=xmlTagError,xmlTagName,xmlNamespace,xmlAttribPunct,@xmlTagHook
    ...
    " </tag>
    "
    syn region   xmlRegion start=+<\z([^ /!?<>"']\+\)+ skip=+<!--\_.\{-}-->+ end=+</\z1\_\s\{-}>+ matchgroup=xmlEndTag end=+/>+ fold contains=xmlTag,xmlEndTag,xmlCdata,xmlRegion,xmlComment,xmlEntity,xmlProcessing,@xmlRegionHook,@Spell keepend extend
    ...
    "
    syn region   xmlTag start=+<[^ /!?<>"']\@=+ end=+>+ contains=xmlTagError,xmlTagName,xmlAttrib,xmlEqual,xmlString,@xmlStartTagHook

    syn region   xmlEndTag start=+</[^ /!?<>"']\@=+ end=+>+ contains=xmlTagError,xmlTagName,xmlNamespace,xmlAttribPunct,@xmlTagHook
...
if exists('g:xml_syntax_folding')
    ...
    syn region  xmlComment start=+<!+ end=+>+ contains=xmlCommentPart,xmlCommentError extend fold
...
syn region  xmlCommentPart start=+--+ end=+--+ contained contains=xmlTodo,@xmlCommentHook,@Spell
...
"
syn region    xmlCdata start=+<!\[CDATA\[+ end=+]]>+ contains=xmlCdataStart,xmlCdataEnd,@xmlCdataHook,@Spell keepend extend
...
if exists('g:xml_syntax_folding')
    ...
    syn region  xmlDocType matchgroup=xmlDocTypeDecl start="<!DOCTYPE"he=s+2,rs=s+2 end=">" fold contains=xmlDocTypeKeyword,xmlInlineDTD,xmlString
else
